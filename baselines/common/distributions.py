...
class CategoricalPd(Pd):
    ...
    def neglogp(self, x):
        ...
        # one_hot_actions = tf.compat.v1.one_hot(x, self.logits.get_shape().as_list()[-1]) # original version
        one_hot_actions = tf.compat.v1.one_hot(x, tf.compat.v1.shape(self.logits)[-1])
